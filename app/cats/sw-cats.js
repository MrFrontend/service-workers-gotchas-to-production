...
let CURRENT_CACHES = {
  offline: 'offline-cats-v' + CACHE_VERSION
};
...
self.addEventListener('fetch', function (event) {
  ...
  // console.log('%c Cats worker: ', 'color:black; background-color: #33cc33', 'Handling fetch event for ', event.request.url);
  ...
  event.respondWith(
    ...
    caches.match(event.request).then(function(response) {
      if (response) {
        // Cache hit !
        console.log('%c Cats worker: ', 'color:black; background-color: #33cc33', 'Cache hit !', event.request.url);
        return response;
      }
      ...
      return fetch(event.request).then(function(response) {
        console.log('%c Cats worker: ', 'color:black; background-color: #33cc33', 'Not found ! ', event.request.url);
        return response;
      }).catch(function(error) {
