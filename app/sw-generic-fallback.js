...
/**
 * Delete old caches under the control of this service worker.
 * 
 * @return  {Promise}   A promise which resolves when all the old caches have been deleted.
 */
...
self.addEventListener('activate', function (event) {
  ...
  event.waitUntil(
    Promise.all([
      ...
      cacheAssets(CURRENT_CACHES.images, imagesToPreload)
    ])
  );
...
/**
 * Returns a generic fallback for not found assets.
 */
function genericFallback(request) {
    ...
    caches.match(event.request).then(function(response) {
      ...
      return fetch(event.request).then(function(response) {
        if (response.status == 404) {
          // Not found
          return genericFallback(event.request);
        }
